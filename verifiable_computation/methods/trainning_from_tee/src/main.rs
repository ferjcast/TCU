...
fn main() {
    ...
    let gm_encoded_verifying_key: EncodedPoint = env::read();
    let gm_message: Vec<u8> = env::read();
    let gm_signature: Signature = env::read();

    let global_model: Vec<f64> = env::read(); // Adjust to read a batch of targets
    ...
    let gm_verifying_key = VerifyingKey::from_encoded_point(&gm_encoded_verifying_key).unwrap();
    let gm_is_valid = gm_verifying_key.verify(&gm_message, &gm_signature).is_ok();


    println!("is_valid {}", gm_is_valid);
    let gm_ds_hash: Vec<u8> = hash_dataset(&inputs, &targets);

    let mut gm_hash_validity = false;

    if gm_ds_hash == gm_message {
        gm_hash_validity = true;
        println!("random_matrix_hash and message are equal.");
    }
    ...
    //added global model initialization    
...
*/
    ...
    env::commit(&(flattened_params, hash_validity, is_valid, gm_hash_validity, gm_is_valid)); 
    println!(

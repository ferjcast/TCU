...
fn main() {
    ...
    let gm_journal: Journal = env::read();
    let gm_image_id: risc0_zkvm::sha::Digest = env::read();

    
    let mut gm_hash_validity = true;
    let mut gm_is_valid = true;


    env::verify(gm_image_id, &gm_journal.bytes).unwrap();



    let (gm_flattened_params, hash_validity, is_valid): (Vec<f64>, bool, bool) = gm_journal.decode().unwrap();
    ...
    //use global model
    nn.unflatten(&gm_flattened_params);
